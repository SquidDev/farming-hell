...
export const ShortRequirements: FunctionComponent<{ requirements: Requirements, store: Store }> = ({ requirements: { items, qp, exp, expBonus }, store }) => {
  const itemList: Array<ReactElement<unknown>> = [...items.keys()].sort(byPriority(store.itemLookup)).map(key => {
    const value = items.get(key)!;
    return <ItemWithAmount key={key} item={key} store={store} amount={value} />;
  });
  return <>
